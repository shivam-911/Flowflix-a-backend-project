...
const getVideoById = asyncHandler(async (req, res) => {
    ...
    const video = await Video.findById(videoId)
        ...
        .select("title description thumbnail duration views videoFile createdAt isPublished");
    ...
    if(!(video.isPublished ===true || video.owner._id.toString() === req.user._id.toString())){
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if(video.owner.toString() !== req.user._id.toString()){
        throw new ApiError(403,"You are not authorized to make chnages in this video")
    }
